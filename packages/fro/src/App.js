...
const AccountPut = () => {
  ...
  const onSubmit = async ({ 账号, 密码 }) => {
    ...
    const 返回值 = await apiPost("/put", 提交表).catch((err) =>
      form.setError("账号", err.message)
    );
    form.reset();
    // form.setValue("账号", "");
    // form.setValue("密码", "");
...
const apiFetch = async (path, body = null) =>
  await fetch(apiBase + path, {
    ...
    .then((e) => e.text())
    .then(jsonpParse);
const apiGet = async (api, search = {}) =>
  await apiFetch(
    !search ? api : `${api}?${new URLSearchParams(search).toString()}`
  );
...
const JSON齐化 = (json) => {
  ...
  const 表共键补齐 = (表) =>
    Object.fromEntries(共键列.map((键) => [键, 表[键]]));
    ...
    <div>
      <div>
        <div className="status">
          ...
          <span className="time"># 刷新于：{new Date().toISOString()}</span>
          <ActionButton onClick={update} text="刷新" />
          <ActionButton onClick={copy} text="JSON复制" />
        </div>
        <H3 name={name} />
      </div>
...
export const ApiAuthForm = () => {
  ...
  const onSubmit = async ({ 用户名, 密码 }) => {
    const 令牌 = await apiPost("/login", { 用户名, 密码 }).catch((err) =>
      form.setError("账号", err.message)
    );
