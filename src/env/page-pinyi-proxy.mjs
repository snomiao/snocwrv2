...
async function 品易代理API余额警告() {
    const json = await fetch(process.env.PINYI_BALANCE_URL).then(e => e.json());
    const balance = json.data?.balance ?? json.ret_data?.balance;
    console.log(`品易代理API余额为：${balance}`);
    if (balance <= 5) console.warn(" 品易代理API余额不足5元");
    if (balance <= 1) throw new Error("品易代理API余额不足1元");
}
...
export async function 代理IP提取入库() {
    ...
    const IP提取地址 =
        ...
        "http://tiqu.pyhttp.taolop.com/getip?count=1&neek=15779&type=2&yys=0&port=2&sb=&mr=1&sep=0&ts=1&ys=1&cs=1&time=3";
    ...
    if (pr?.msg?.match?.(/请添加白名单(.*)/)) {
        ...
        const ip = m[1];
        const json = await fetch(process.env.PINYI_WHITEIP_URL + ip).then(e =>
            e.json()
        );
